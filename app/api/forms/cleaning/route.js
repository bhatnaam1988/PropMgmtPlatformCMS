...
export async function POST(request) {
  try {
    ...
    await emailService.sendEmail({
      ...
      subject: `New Cleaning Service Request: ${sanitizedData.serviceType}`,
      html: emailHtml
    });

    logger.secureLog('Cleaning service request submitted', { email: sanitizedData.email, type: sanitizedData.serviceType });
    ...
    logger.error('Cleaning form submission error', error);
    return NextResponse.json(
      { success: false, error: 'Failed to submit cleaning service request. Please try again later.' },
      { status: 500 }
    );
  }
}
