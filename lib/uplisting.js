...
export function calculatePricing(calendarData) {
  ...
  if (days.length === 0) {
    return {
      available: true, // Allow booking if no calendar data
      averageRate: 0,
      totalNights: 0,
      total: 0,
      unavailableDates: [],
  ...
  days.forEach(day => {
    const attrs = day.attributes || day;
    if (!attrs.available || attrs.closed) {
      unavailableDates.push(attrs.date);
    } else {
      totalRate += parseFloat(attrs.rate || attrs.price || 0);
      availableNights++;
    }
  });
  ...
  return {
    ...
    total: totalRate,
    unavailableDates,
    currency: days[0]?.attributes?.currency || days[0]?.currency || 'CHF',
