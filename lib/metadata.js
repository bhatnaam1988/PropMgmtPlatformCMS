...
export function generatePropertyMetadata(property) {
  ...
  return generateMetadata({
    ...
    image,
    type: 'website', // Changed from 'product' - OpenGraph only supports: website, article, profile
    keywords,
  });
}
...
export function generateBlogListMetadata() {
  return generateMetadata({
    title: 'Travel Blog',
    description: 'Discover travel tips, local insights, and stories from the Swiss Alps. Read about hiking trails, winter sports, local culture, and vacation planning advice.',
    path: '/blog',
    keywords: ['Swiss Alps blog', 'Grächen travel blog', 'mountain travel tips', 'alpine adventure stories', 'Switzerland travel guide'],
  });
}
...
export function generateBlogPostMetadata(post) {
  if (!post) return generateMetadata({ title: 'Blog Post Not Found' });

  const title = post.title;
  const description = post.excerpt || post.seo?.metaDescription || post.title;
  const image = post.mainImage?.asset?.url || post.seo?.ogImage?.asset?.url || defaultImage;
